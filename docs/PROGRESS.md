...
## 2026-08-19: Deep on-device debug (moto g34, Turnip WN-1.06, extracted2 data)
- Validated the v3 config install end-to-end on device: <user>/redahm.toml contains the full tuned profile; launcher graphics-profile overlay only touches disable_motion_blur (profile QUALITY=1 selected in UI -> disable_motion_blur=false observed; PERFORMANCE -> true).
- Boot is clean: Turnip init OK, 1600x720 FIFO swapchain, zero engine errors/warnings, no tombstones. Texture streaming normal (tex_miss only during scene loads); no log evidence of texture corruption.
- Measured phases (PERF line):
  - Boot/movies/menu: 18-37 fps, exec_busy 4-11% - healthy.
  - Scene load ("open is_paradiso"): 20-25s stall, fps=0, exec_busy 87-94%, 277 tex_miss + 32-90 pipe_miss on first entry, ~0 afterwards (shader/pipeline storage under <user>/cache/shaders persists across runs).
  - Gameplay: ~2 fps sustained, exec_busy=100%, ~320 draws/s, ~145k verts/s, caches 100% hot (pipe_miss=0, tex_miss=0).
- A/B test: async_shader_compilation=true + vulkan_async_skip_incomplete_frames=false -> identical gameplay fps (~2) and similar scene-load stall; reverted to the shipped v3 tuning (async off).
- Bottleneck isolation: during gameplay the "GPU Commands" worker thread consumes only 7-24% CPU while exec_busy reads 100% -> the worker is blocked waiting on the GPU (exec time is spent inside ExecutePrimaryBuffer GPU waits, not CPU work); exec_busy=100% in this regime means GPU-saturated, not CPU-bound. The guest render thread runs at 65-100% CPU producing commands.
