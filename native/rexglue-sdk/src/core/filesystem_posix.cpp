...
std::filesystem::path GetExecutablePath() {
  // On Android there is no per-app executable to readlink: the process runs
  // under app_process. Native libraries live in the APK's native library
  // directory, which is also where the GPU plugin is staged, so resolve there
  // instead.
  auto lib_dir = rex::GetAndroidNativeLibraryDir();
  if (lib_dir.empty()) {
    return "/data/local/tmp/redahm";
  }
  return lib_dir;
}
#else
